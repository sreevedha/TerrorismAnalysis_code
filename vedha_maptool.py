...
def create_app_ui():
  ...
  main_layout = html.Div(style={'backgroundColor':colors['background']},children=[
       html.H1('TERRORISM ANALYSIS USING VISUAL SYNTHESIS', id='Main_title',style={'text-align':'center','font-family':'cursive'}),
  ...
  html.Div(id = "graph-object", children ="Loading map....")
  ],
  )
...
def update_app_ui(Tabs, month_value, date_value,region_value,country_value,state_value,city_value,attack_value,year_value,chart_year_selector, chart_dp_value, search,
    ...
    if Tabs == "Map":
        ...
        fig = px.area(chart_df, x="iyear", y ="count", color = chart_dp_value,template='plotly_dark',title='Analysis using Area Graph')
